...
type HighLoadStabilityTests() =
    ...
    member this.MemoryPressureUnderHighLoad_ShouldNotCauseFailure() =
        ...
        | Ok newDb ->
            ...
            try
                let keys = Array.init dataSize (fun i -> Encoding.UTF8.GetBytes $"key_{i:D6}")

                let values =
                    Array.init dataSize (fun i ->
                        let valueSize = 100 + (i % 400)

                        Encoding.UTF8.GetBytes($"value_{i}_" + new string ('x', valueSize)))
                ...
                for batchStart in 0..batchSize .. dataSize - 1 do
                    ...
                    for i = batchStart to batchEnd do
                        let setResult = db.Set(keys[i], values[i]) |> Async.RunSynchronously
                    ...
                    printfn $"Completed batch {batchIndex + 1}, memory: {GC.GetTotalMemory true / 1024L / 1024L}MB"
                ...
                for i = 0 to min 1000 (dataSize - 1) do
                    ...
                    | Ok(Some value) ->
                        if not (System.Linq.Enumerable.SequenceEqual(value, values[i])) then
                ...
                printfn $"Successfully completed memory pressure test for data size {dataSize}"

            with ex ->
                Assert.Fail $"Memory pressure test failed for data size {dataSize}: {ex.Message}"
    ...
    member this.ConcurrentOperationsUnderHighLoad_ShouldNotDeadlock() =
        ...
        | Ok newDb ->
                ...
                async {
                    try
                        for i = 0 to (operationCount / concurrentTasks) - 1 do
                            ...
                            let value =
                                Encoding.UTF8.GetBytes($"concurrent_value_{keyIndex}_" + new string ('c', 150))
                            ...
                            if i % 2 = 0 then
                                ...
                                match setResult with
                                | Ok() -> ()
                                | Error msg -> failwith $"Task {taskId}: Write failed for key {keyIndex}: {msg}"
                            else
                                let readKeyIndex = keyIndex - 1

                                if readKeyIndex >= 0 then
                                    let readKey = Encoding.UTF8.GetBytes $"concurrent_key_{readKeyIndex:D6}"
                                    ...
                                    | Ok(Some _) -> ()
                                    | Ok None -> ()
                                    | Error msg -> failwith $"Task {taskId}: Read failed for key {readKeyIndex}: {msg}"
            ...
            try
                let tasks = [ 0 .. concurrentTasks - 1 ] |> List.map (fun id -> operation id ())
                ...
                let taskArray =
                    asyncTasks |> Array.ofList |> Array.map (fun (t: Task<unit>) -> t :> Task)

                let allCompleted = Task.WaitAll(taskArray, int timeout.TotalMilliseconds)
            ...
            | :? System.AggregateException as ae ->
                ...
                let deadlockIndicators = [ "deadlock"; "timeout"; "hung"; "blocked" ]

                let hasDeadlock =
                    innerExceptions
                    |> Seq.exists (fun ex ->
                        deadlockIndicators
                        |> List.exists (fun indicator -> ex.Message.ToLower().Contains indicator))

                if hasDeadlock then
                    Assert.Fail $"Deadlock detected in concurrent operations: {ae.Message}"
                else
                    Assert.Fail $"Concurrent operations failed: {ae.Message}"

            | ex -> Assert.Fail $"Unexpected exception in concurrent operations: {ex.Message}"

        | Error msg -> Assert.Fail $"Failed to open DB for concurrent operations test: {msg}"
