            ...
            if location < tail || location >= head then
                ...
                try
                    ...
                    if bytesAvailable < 8L then // Need at least 8 bytes for two int32 lengths
                        Logger.Warn $"Insufficient data at location {location}, only {bytesAvailable} bytes available"
                        ...
                        if keyLength <= 0 || keyLength > 1024 * 1024 then // Reasonable upper bound
                            Logger.Warn $"Invalid key length {keyLength} at location {location}"

                            None
                        elif int64 keyLength > bytesAvailable - 8L then // Not enough data for key + value lengths
                            Logger.Warn $"Key length {keyLength} exceeds available data at location {location}"
                            ...
                            if valueLength < 0 || valueLength > 10 * 1024 * 1024 then // 10MB upper bound
                                Logger.Warn $"Invalid value length {valueLength} at location {location}"

                                None
                            elif int64 valueLength > fileStream.Length - fileStream.Position then
                                Logger.Warn
