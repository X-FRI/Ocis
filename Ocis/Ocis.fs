...
type OcisDB
            ...
            | Some level0SSTables when level0SSTables.Length >= L0_COMPACTION_THRESHOLD ->
                ...
                if not selectedCandidates.IsEmpty then
                    ...
                    | Ok(newSSTblOption, liveEntries) ->
                        do! OcisDB.updateSSTablesAfterCompaction (dbRef, newSSTblOption, sstblsToMerge, [])
        ...
        async {
            while true do
                ...
                | RecompactionForRemapped remappedLocations ->
                    ...
                    for (level, finalSSTblList, levelRecompactedCount) in recompactionTasks do
    ...
    /// </summary>
    static member private flushAgentLoop (dbRef: OcisDB ref) (mailbox: MailboxProcessor<FlushMessage>) =
        async {
            while true do
                let! msg = mailbox.Receive()
                ...
                | FlushMemtable memtblToFlush ->
                    ...
                    try
                        ...
                        | None ->
                            Logger.Fatal $"Failed to open flushed SSTable at {flushedSSTblPath}. Data may be lost!"
                            ...
                            failwith $"SSTable flush failed for {flushedSSTblPath} - data integrity compromised"
                    with ex ->
            ...
            while true do
                ...
                | TriggerGC ->
                    ...
                    try
                        ...
                        Logger.Info $"GC Agent: Collected {liveLocations.Count} live value locations"

                        if liveLocations.IsEmpty then
                            Logger.Info "GC Agent: No live locations found, skipping garbage collection"
                        else
                            ...
                            if useIncrementalGC then
                                ...
                                let! gcResult =
                                    Valog.CollectGarbageIncremental(dbRef.Value.ValueLog, liveLocations, gcConfig)
                                ...
                                | Ok(state, newValogOption) when state.IsCompleted ->
                                    ...
                                    lock dbRef.Value (fun () ->
                                        if not (obj.ReferenceEquals(dbRef.Value.ValueLog, null)) then
                                            try
                                ...
                                Logger.Info "GC Agent: Using legacy synchronous garbage collection"
                                ...
                                | Some(Ok(newValog, remappedLocations)) ->
                                    ...
                                    Logger.Info $"Legacy GC completed: {remappedLocations.Count} locations remapped"
                ...
                | Ok() ->
                    ...
                    |> Result.bind (fun valog ->
                        ...
                        |> Result.bind (fun wal ->
                            ...
                            let flushAgent = MailboxProcessor.Start(OcisDB.flushAgentLoop dbRef)
    ...
    member this.Set(key: byte array, value: byte array) : Async<Result<unit, string>> =
        async {
            try
